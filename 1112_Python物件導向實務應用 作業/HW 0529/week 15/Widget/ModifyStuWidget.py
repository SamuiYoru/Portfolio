...
class ModifyStuWidget(QtWidgets.QWidget):
    def __init__(self,client):
        ...
        modify_widget = ModifyWidget(client)
...
class ModifyWidget(QtWidgets.QWidget):
    def __init__(self,client):
        ...
        self.modify_widget = ModifyinfoWidget(client)
        ...
        if(confirm_flag == 1):
            if(self.subject!="" and self.score!=""):
                if(int(self.score) >= 0):
                    ...
                    message = {'command': 'modify', 'parameters': {'name': self.editor_label_Name.text(), 'scores': self.scores_dict}}
                    self.send_command = Connector(message, self.client)
                    self.send_command.start()
