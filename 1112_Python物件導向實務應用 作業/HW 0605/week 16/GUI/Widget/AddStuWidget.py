...
class AddStuWidget(QtWidgets.QWidget):
    ...
    def query_action(self):
        if(self.editor_label_Name.text()!=""):
            ...
            query_data = {"name": self.editor_label_Name.text()}
            self.execute_query = ExecuteCommand(command="query", data=query_data)
            self.execute_query.start()
            # self.send_command.start()
            self.execute_query.return_sig.connect(self.query_result)
        else:#未輸入學生姓名就按query
    ...
    def send_action(self):
        if self.scores:
            ...
            # add_data =  {'name': self.editor_label_Name.text(), 'scores': self.scores}
            # self.send_command = Connector(message, self.client)
            add_data =  {'name': self.editor_label_Name.text(), 'scores': self.scores}
            self.send_command = ExecuteCommand(command="add", data=add_data)
            self.send_command.start()
